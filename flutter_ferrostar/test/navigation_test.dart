...
class MockFlutterNavigationController extends Mock implements FlutterNavigationController {}
class MockFlutterNavState extends Mock implements FlutterNavState {}

// Fake classes for fallback
class FakeRoute extends Fake implements Route {}
class FakeFlutterNavigationControllerConfig extends Fake implements FlutterNavigationControllerConfig {}
...
class FakeFlutterNavigationController extends Fake implements FlutterNavigationController {}
...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockApi.rust_arc_increment_strong_count_FlutterNavState).thenReturn((ptr) {});
    when(() => mockApi.rust_arc_decrement_strong_count_FlutterNavState).thenReturn((ptr) {});
    when(() => mockApi.rust_arc_increment_strong_count_FlutterNavigationController).thenReturn((ptr) {});
    when(() => mockApi.rust_arc_decrement_strong_count_FlutterNavigationController).thenReturn((ptr) {});
  });
  ...
  group('FlutterNavigationController', () {
    test('newInstance returns controller', () async {
      ...
      when(() => mockApi.crateApiNavigationFlutterNavigationControllerNew(
        route: any(named: 'route'),
        config: any(named: 'config'),
      )).thenAnswer((_) async => mockController);

      final controller = await FlutterNavigationController.newInstance(route: route, config: config);
      expect(controller, equals(mockController));
      
      verify(() => mockApi.crateApiNavigationFlutterNavigationControllerNew(
        route: route,
        config: config,
      )).called(1);
    });
    ...
    test('methods call mock implementation', () async {
      ...
      when(() => controller.getInitialState(location: any(named: 'location')))
          .thenAnswer((_) async => mockState);
      when(() => controller.advanceToNextStep(state: any(named: 'state')))
          .thenAnswer((_) async => mockState);
  ...
  group('FlutterNavState', () {
    test('tripState returns idle', () async {
      ...
      when(() => state.tripState()).thenAnswer((_) async => const FlutterTripState.idle());
